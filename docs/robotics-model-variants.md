...
| --- | --- | --- | --- | --- | --- | --- |
| MinimalRealistic | Represent a small but plausible industrial robot implementation. | Baseline reference-quality robot model variant for early integration. | Expected to pass checks for the model areas it claims to implement. | Identity, basic motion device structure, selected axes, selected runtime telemetry. | Rich diagnostics, advanced task control, condition monitoring, extended asset data. | No. |
