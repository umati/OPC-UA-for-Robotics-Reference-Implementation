...
export default function Robot3DViewport({ axes, snapshots, motionInventory, live, imageUrl }: Props) {
  ...
  useEffect(() => {
    const element = host.current; if (!element || state.mappingStatus !== 'matched' || failed) return;
    ...
    const actualRotation = (item: THREE.Group) => { const axis = item.userData.rotationAxis as THREE.Vector3; const component = axis.x ? 'x' : axis.y ? 'y' : 'z'; return item.rotation[component]; };
    const setActualRotation = (item: THREE.Group, value: number) => { const axis = item.userData.rotationAxis as THREE.Vector3; const component = axis.x ? 'x' : axis.y ? 'y' : 'z'; item.rotation[component] = value; };
  ...
  if (failed || state.mappingStatus !== 'matched') return <div className="robot3d-panel">{fallback}<div className="robot3d-explanation">{failed ? 'WebGL could not be initialized.' : state.explanation}</div><span className="robot3d-status unavailable">Unavailable</span></div>;
