...
export function createGenericSixAxisRobot(): RobotVisual {
  ...
  const axes: THREE.Vector3[] = [new THREE.Vector3(0, 1, 0), new THREE.Vector3(0, 0, 1), new THREE.Vector3(0, 0, 1), new THREE.Vector3(1, 0, 0), new THREE.Vector3(0, 0, 1), new THREE.Vector3(1, 0, 0)];
  const lengths = [0.58, 0.85, 0.72, 0.58, 0.42, 0.28];
  let parent = base;
  for (let index = 0; index < 6; index += 1) {
    const group = new THREE.Group(); group.name = `Joint${index + 1}`; group.position.y = index === 0 ? 0.24 : lengths[index - 1]; group.userData.rotationAxis = axes[index];
